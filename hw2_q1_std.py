...
#b
#i
standardized_dataset=training_dataset
standardized_test_dataset=test_dataset

standardized_dataset=(training_dataset-training_dataset.mean())/((training_dataset).std())
standardized_test_dataset=(test_dataset-training_dataset.mean())/(training_dataset).std()

print(standardized_dataset)
print(standardized_test_dataset)
covariance=standardized_dataset.cov()
eigen_values, eigen_vectors=LA.eig(covariance)
print("Covariance Matrix size", covariance)
cov_test=standardized_test_dataset.cov()
...
for k in range(len(p)):
    ...
    test_result = np.matmul(feature_vector, np.transpose(standardized_test_dataset))
    print(test_result, "TEST")
    result=np.matmul(feature_vector,np.transpose(standardized_dataset))
    print(result.shape,"traingi")
